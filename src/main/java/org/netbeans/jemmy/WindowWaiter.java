...
public class WindowWaiter extends Waiter<Window, Void> implements Timeoutable {
     ...
     * @throws InterruptedException
     */
    public static void waitWindowCount(final Window owner, final ComponentChooser ch, final int count)
            throws InterruptedException {
     ...
     */
     public static int countWindows(final Window owner, final ComponentChooser ch) {
        return new QueueTool().invokeAndWait(new QueueTool.QueueAction<Integer>(null) {
            ...
            public Integer launch() {
                ...
                for( int i = 0 ; i < windows.length ; i++ ) {
                   if ( ch.checkComponent(windows[i]) )
                      count++;
                }
                return count;
            }
